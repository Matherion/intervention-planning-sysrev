...
renderChecklist <- function(extractionScriptTree,
  ...
  for (i in 1:nrow(definitions)) {
    ...
    #matchString <- paste0("<a[^>]*>[^<]*<\\/a|(", definitions[i, definitionsTermsCol], ")");
    matchString <- paste0("<a[^>]*>[^<]*<\\/a(*SKIP)(*F)|(", definitions[i, definitionsTermsCol], ")");
    ...
    # checklistBodyXML <- htmlParse(checklistBody, useInternalNodes = TRUE);
    # 
    # checklistBodyNodes <- xpathApply(checklistBodyXML, "////*");
    # invisible(sapply(checklistBodyNodes, function(n) {
    #   print(xmlValue(n));
    #   newNodeText <- gsub(matchString, definitionToolTip,
    #                       xmlValue(n),
    #                       ignore.case=TRUE);
    #   xmlValue(n) <- newNodeText;
    #   print(xmlValue(n));
    # }));
    # cat0("definition", i, "\n");
    # browser();
    # checklistBody <- toString(checklistBodyNodes);

    # checklistBodyProcessed <- xpathApply(checklistBodyXML, "////*",
    # checklistBodyProcessed <- xpathApply(checklistBodyXML, "////text()",
    #                                      function(node) {
    #                                        xmlValue(node) <-
    #                                          gsub(matchString, definitionToolTip, xmlValue(node), ignore.case=TRUE);
    #                                      });
          
    # checklistBodyProcessed <- xpathApply(checklistBodyXML, "//text()",
    #                                      gsub, pattern=matchString,
    #                                      replacement=definitionToolTip,
    #                                      ignore.case=TRUE);
    # checklistBody <- toString(checklistBodyProcessed);
    #browser();
    # checklistBody <- gsub(matchString, definitionToolTip,
