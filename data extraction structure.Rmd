...
extractionScriptBody <-
  paste0(sapply(Traverse(extractionScriptTree,
                ...
                function(x) {
                  ...
                  if (length(Get(x$children, 'isLeaf')) > 0) {
                    
                    res <- paste0(repStr("#", 80),
                                  "\n### Activity (at least some children are properties)\n",
                    ...
                    res <- paste0(res,
                                  paste0(sapply(Traverse(x$children, traversal="pre-order",
                                                pruneFun=isLeaf), function(xx) {
                                           if (isLeaf(xx)) {
                                             return(paste0("          ", xx$name));
                                           } else {
                                             return(isLeaf(xx));
                                           }
                                         }), collapse=" = NA\n        ,\n"),
