...
leafExtractionscript <- function(node, lvlSpaces="  ",
  ...
  if (!isRepeating(node[[repeatingColName]]) && !isRecursive(node[[recursionColName]])) {
    ...
    rslt <- paste0(rslt, "\n",
                   ...
                   "\n");
    addScoring <- TRUE;
  } else if (isRepeating(node$Get(repeatingColName)) && !isRecursive(node$Get(recursionColName))) {
    ...
    rslt <- paste0(rslt, "\n",
                   ...
                   lvlSpaces, "###\n",
                   "\n",
                   lvlSpaces, "    list('", node[[ownerColName]], "1' = NA,\n",
                   lvlSpaces, "         '", node[[ownerColName]], "2' = NA)\n",
                   "\n");
    addScoring <- TRUE;
    ...
    rslt <- paste0(rslt, "\n",
                   ...
                   paste0(strwrap(node[[descriptionColName]],
                          ...
                          collapse="\n"), "\n",
                   lvlSpaces, "###\n",
                   lvlSpaces, "### This node is recursive! Scroll down to the section marked",
                   lvlSpaces, "### 'RECURSIVE ELEMENTS' and select the lines corresponding to ",
                   lvlSpaces, "### recursive element '", node[[recursionColName]], "'.",
                   lvlSpaces, "### Copy-paste those here.");
  }
...
FindNode(extractionScriptTree, name=recursionsName)$Do(function(node) {
  if (isRoot(node)) {
    node$extractionscript <- paste0("res <- Node$new('", recursionsName, "');");
  } else {
    node$extractionscript <- paste0(returnPathToRoot(node$parent),
                                    "$AddChild('", node$name, "');");
  }
  });
