...
#                "\n\n");
  ...
  if (!isRepeating(node[[repeatingColName]]) && !isRecursive(node[[recursionColName]])) {
    ...
    currentElementName <- ifelse(isRepeating(node[[repeatingColName]]),
                                 ...
                                 node$name)
    currentElementPath <- paste0(returnPathToRoot(node$parent),
                                 "$", currentElementName);
    ...
    rslt <- paste0(rslt,
                   "\n",
                   lvlSpaces, "    recursiveElementPlaceholder <- ", currentElementPath,
...
### recursive extractionscript fragments
###################################################################################
...
###################################################################################
### Add extractionscript bits to the tree, second for the
...
renderChecklist <- function(extractionScriptTree,
  ...
  for (i in 1:nrow(definitions)) {
    ...
    definitionToolTip <- paste0('<a class="metabefor-term-definition" href="#" data-toggle="tooltip" title="',
  ...
  if (!is.null(outputFile)) {
    writeLines(paste0(checklistBody, collapse=""), outputFile);
  }
